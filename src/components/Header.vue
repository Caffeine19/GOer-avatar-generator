...
<template>
  <div
    ...
    <div class="flex items-center space-x-3">
      <Avatar class="xl:w-16 xl:h-16 md:w-12 md:h-12 md:-mt-3 w-8 h-8 -mt-2" :color="logoColor"></Avatar>
      <h1 class="cartograph-cf-regular-italic text-gradient xl:text-2xl md:text-xl text-lg" :style="{
    ...
    <div class="xl:flex items-stretch hidden space-x-1">
      <div v-for="(button, index) in HeaderButtonOptions" :key="index" class="relative flex items-stretch space-x-1">
        <Modified v-if="button.title == '保存' && modified" class="top-2 left-2"></Modified>
        <ButtonPopover :content="button.title">
          <HeaderButton :title="button.title" :icon="button.icon" :action="button.action" :showTitle="false">
          </HeaderButton>
        </ButtonPopover>
        <Divider direction="vertical" percent="h-3/5" />
      </div>
      <ButtonPopover :content="theme == THEME.LIGHT ? '夜~' : '日！'">
        <HeaderButton :icon="theme == THEME.LIGHT ? 'ph-moon-stars' : 'ph-sun'"
          :title="theme == THEME.LIGHT ? '夜~' : '日！'" :action="toggleTheme" :showTitle="false"></HeaderButton>
      </ButtonPopover>
    </div>
    <div class="relative xl:hidden">
      <HeaderButton icon="ph-list-dashes" title="菜单" :action="toggleMenu" :showTitle="false"></HeaderButton>
    ...
    <div
      ...
      <div v-for="(button, index) in HeaderButtonOptions" :key="index" class="space-y-1">
        ...
        <HeaderButton :title="button.title" :icon="button.icon" :action="button.action"></HeaderButton>
        <Divider />
      </div>
      <HeaderButton :icon="theme == THEME.LIGHT ? 'ph-moon-stars' : 'ph-sun'" :title="theme == THEME.LIGHT ? '夜~' : '日！'"
        :action="toggleTheme"></HeaderButton>
    </div>
  </div>
</template>
<style></style>
